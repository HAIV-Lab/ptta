...
class ResNet50FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        # 对输入数据进行必要的检查和处理，这里假设输入x是四维张量 (batch_size, channels, height, width)
        if self.model_name == "WideResNet":
            ...
            features = out.view(-1, self.model.nChannels)
            output =  self.model.fc(features)
        else:
            assert len(x.shape) == 4, "输入数据应该是四维张量"
            # 前向传播，得到特征输出
...
class PTTA(nn.Module):
    def __init__(self, model, optimizer, e_margin=math.log(1000)*0.40, d_margin=0.05, loss2_weight=3, 
        ...
        self.feature_extractor_state = deepcopy(self.model.feature_extractor.state_dict())
        self.fc_state = deepcopy(self.model.fc.state_dict())
    ...
    def reset(self, alpha=0.3):
        self.model.feature_extractor.load_state_dict(self.feature_extractor_state)
        self.model.fc.load_state_dict(self.fc_state)
    ...
    def forward_and_adapt_eata(self, x):
        ...
        if self.current_model_probs is not None:
            ...
            if self.memory_bank.queue_size > 0:
                if outputs[filter_ids_1][filter_ids_3].size(0) > 0:
                    self.memory_bank.update(x[filter_ids_1][filter_ids_3], features[filter_ids_1][filter_ids_3].clone().detach(), outputs[filter_ids_1][filter_ids_3].clone().detach())
                else:
        ...
        if self.memory_bank.num_features_stored > 0:
            pred_labels, probs, img, grads_m = self.memory_bank.refine_predictions(features.clone().detach())
            alpha = self.alpha
            x = x * alpha + img.cuda() * (1 - alpha)
            probs = outputs.softmax(dim=-1) * alpha + probs.cuda() * (1 - alpha)
            outputs2, features = self.model(x)
...
class FeatureBank:
    ...
    def update(self, x, features, logits):
        ...
        if (self.features is None or self.probs is None) or self.queue_size == 0:
