...
class FeatureBank:
    ...
    def update(self, x, features, logits):
        ...
        if (self.features is None or self.probs is None) or self.queue_size == 64:
            self.features = torch.zeros(self.queue_size, features.size(1)).cpu()
            self.probs = torch.zeros(self.queue_size, probs.size(1)).cpu()
            self.image_bank = torch.zeros(self.queue_size, x.size(1), x.size(2), x.size(3)).cpu()
        idxs_replace = torch.arange(start, end).cuda() % self.features.size(0)
        self.features[idxs_replace, :] = features.cpu()
        self.probs[idxs_replace, :] = probs.cpu()
    ...
    def refine_predictions(self, features):
        ...
        return pred_labels, probs.cuda(), images.cuda(), grads
