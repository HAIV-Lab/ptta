...
import copy
from utils.utils_cpl import find_elem_idx_BinA, PoolsAggregation
...
class InstanceSelector(object):
    ...
    def _prepare_items_attrs(self, PL_labels, outputs, method, max_num="all"):   
        ...
        if max_num == "all":
            # Check if PL_labels is empty
            if PL_labels.numel() == 0:
                max_num = 0
            else:
        ...
        for i in range(max_num):
            ...
            # uncs[mask] = torch.inf
            uncs[mask] = torch.tensor(float('inf'), device=self.device, dtype=uncs.dtype)
            # conf_all[torch.arange(0, conf_all.shape[0]), max_idx] = -torch.inf
            conf_all[torch.arange(0, conf_all.shape[0]), max_idx] = torch.tensor(float('-inf'), device=self.device, dtype=conf_all.dtype)
        ...
        if labels_:
            ...
            labels_ = torch.empty(0, max_num, dtype=torch.long, device=self.device)
            uncs_ = torch.empty(0, max_num, dtype=torch.float, device=self.device)
    ...
    def select_topk_for_eachcls(self, PL_labels, indexs_all, output_all, 
        ...
        return selected_idxs, {"top1_acc": 0}
