...
if __name__ == '__main__':
    ...
    elif args.algorithm == 'ptta_cpl_noise':
        import utils.ptta_cpl_noise as ptta_cpl_noise
        subnet = eata.configure_model(subnet)
        params, param_names = eata.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_cpl_noise.CPL(subnet, optimizer, e_margin=args.e_margin, d_margin=args.d_margin, \
                    loss2_weight=args.loss2_weight, queue_size=args.queue_size, \
                    neighbor=args.neighbor)
        print("the model is ptta_cpl_noise")
    elif args.algorithm == 'ptta_cpl_image':
        import utils.ptta_cpl_image as ptta_cpl_image
        subnet = eata.configure_model(subnet)
        params, param_names = eata.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_cpl_image.CPL(subnet, optimizer, e_margin=args.e_margin, d_margin=args.d_margin, \
                    loss2_weight=args.loss2_weight, queue_size=args.queue_size, \
                    neighbor=args.neighbor)
        print("the model is ptta_cpl_image")
    elif args.algorithm == 'ptta_cpl_feature':
        import utils.ptta_cpl_feature as ptta_cpl_feature
        subnet = eata.configure_model(subnet)
        params, param_names = eata.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_cpl_feature.CPL(subnet, optimizer, e_margin=args.e_margin, d_margin=args.d_margin, \
                    loss2_weight=args.loss2_weight, queue_size=args.queue_size, \
                    neighbor=args.neighbor)
        print("the model is ptta_cpl_feature")

    elif args.algorithm == 'ptta_cpl_id':
        import utils.ptta_cpl_id as ptta_cpl_id
        subnet = eata.configure_model(subnet)
        params, param_names = eata.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_cpl_id.CPL(subnet, optimizer, e_margin=args.e_margin, d_margin=args.d_margin, \
                    loss2_weight=args.loss2_weight, queue_size=args.queue_size, \
                    neighbor=args.neighbor)
        print("the model is ptta_cpl_id")
    ...
    args.corruption = "original"
    fisher_dataset, fisher_loader = prepare_test_data(args)
    fisher_dataset.set_dataset_size(args.queue_size)
    fisher_dataset.switch_mode(True, False)
    loss_fn = nn.CrossEntropyLoss()
    from utils.ptta_eata import compute_gradient
    print("use id samples as memory bank")
    for i, (x, y) in enumerate(fisher_loader):
        x, y = x.cuda(), y.cuda()
        outputs = adapt_model.model(x)
        grads = compute_gradient(outputs.clone().detach())
        adapt_model.memory_bank.update(x, grads.clone().detach(), outputs.clone().detach())
        optimizer.zero_grad()
    print("len of memory bank is ", adapt_model.memory_bank.num_features_stored)
    ...
    for corrupt in common_corruptions:
        ...
        from dataset.selectedRotateImageFolder import SelectedRotateImageFolderwithID, te_transforms_imageC
        val_dataset = SelectedRotateImageFolderwithID(validdir, te_transforms_imageC, original=False, rotation=False)
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=64, shuffle=args.if_shuffle, 
