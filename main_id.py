...
if __name__ == '__main__':
    ...
    if args.algorithm == 'tent':
        ...
        import utils.ptta_eata_id as ptta_eata_id
        subnet = eata.configure_model(subnet)
        params, param_names = eata.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_eata_id.PTTA(subnet, optimizer, e_margin=args.e_margin, d_margin=args.d_margin, \
        ...
        import utils.ptta_tent_id as ptta_tent_id
        subnet = tent.configure_model(subnet)
        params, param_names = tent.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_tent_id.PTTA(subnet, optimizer, e_margin=args.e_margin, d_margin=args.d_margin, \
                    loss2_weight=args.loss2_weight, queue_size=args.queue_size, \
                    fisher_alpha=args.fisher_alpha, neighbor=args.neighbor)
        print("the model is ptta_tent")

    elif args.algorithm == 'ptta_deyo':
        import utils.ptta_deyo_id as ptta_deyo_id
        subnet = ptta_deyo_id.configure_model(subnet)
        params, param_names = ptta_deyo_id.collect_params(subnet)
        optimizer = torch.optim.SGD(params, args.learning_rate, momentum=0.9)
        adapt_model = ptta_deyo_id.PTTA(subnet, optimizer, \
                    loss2_weight=args.loss2_weight, queue_size=args.queue_size, \
                    neighbor=args.neighbor)
        print("the model is ptta_deyo_id")
    ...
    for corrupt in common_corruptions:
        if args.exp_type == 'each_shift_reset':
            print("reset")
            adapt_model.reset()
            print(f"adapt_model.memory_bank.num_features_stored is {adapt_model.memory_bank.num_features_stored}")
            # adapt_model.model.load_state_dict(adapt_model.model_state)
            # adapt_model.optimizer.load_state_dict(adapt_model.optimizer_state)
